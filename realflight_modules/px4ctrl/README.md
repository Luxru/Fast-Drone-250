...
功能切换顺序：
**手动模式**：终端打印绿色的"MANUAL_CTRL(L1)"， 控制器不起作用，此时使用pixhawk对应的飞行模式，可以是定高/定点/稳定模式

**定点模式**： 终端打印绿色的"AUTO_HOVER(L2)"，飞控会处于"Offboard"模式（切换到Offboard后自动生效）. 此时控制器会通过代码接管飞控，控制器依据odometry反馈做定点控制，此时不接收你的代码发出的指令.（注意此时无法通过遥控器控制无人机）
**指令控制模式**: 终端打印绿色的"CMD_CTRL(L3)"， 此时控制器允许执行你的代码发给它的飞行指令. 该模式需要你处在定点模式下，使用某两通道解锁，再发送控制指令才能启动，以尽可能进行充分的安全检查.  <br/>

需要实现：

1. 监听offboard模式 改变状态机
2. 监听某*通道切换定点与指令控制

自动起飞降落注意：

**使用两个通道控制，一个通道负责触发（自动回弹），一个通道负责切换模式（起飞/降落）**
1. 起飞需要速度为0
